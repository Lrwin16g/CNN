      ...
      d_weight_(NULL), d_bias_(NULL), d_weight_col_(NULL)
{
    ...
    input_col_ = util::alloc(input_col_h_, input_col_w_);
    input_col_T_ = util::alloc(input_col_w_, input_col_h_);
    dx_col_ = util::alloc(input_col_h_, input_col_w_);

    // 重みを2次元配列に展開した時のサイズ
    weight_col_h_ = channel_ * filter_h_ * filter_w_;
    weight_col_w_ = filter_num_;
    weight_col_ = util::alloc(weight_col_h_, weight_col_w_);
    weight_col_T_ = util::alloc(weight_col_w_, weight_col_h_);

    // 入力と重みの積を格納する行列
    tmp_col_h_ = input_col_h_;
    tmp_col_w_ = weight_col_w_;
    tmp_col_ = util::alloc(tmp_col_h_, tmp_col_w_);

    // 出力の行列形式
    output_col_h_ = input_col_h_;
    output_col_w_ = weight_col_w_;
    output_col_ = util::alloc(output_col_h_, output_col_w_);
    dout_col_ = util::alloc(output_col_h_, output_col_w_);

    // 重み・バイアスの勾配
    d_weight_ = util::alloc(filter_num_, channel_, filter_h_, filter_w_);
    d_bias_ = util::alloc(filter_num_);

    // 重みの勾配の行列形式
    d_weight_col_ = util::alloc(weight_col_h_, weight_col_w_);
}
...
{
    // 入力を2次元配列に展開
    util::im2col(input, input_col_, batch_size_, channel_, height_, width_,
                 filter_h_, filter_w_, stride_, pad_);
...
void Conv2d::backward(double const * const * const * const *dout, double ****dx)
{
    ...
    // 行列をテンソルに戻す
    util::col2im(dx_col_, dx, batch_size_, channel_, height_, width_,
                 filter_h_, filter_w_, stride_, pad_);
}
